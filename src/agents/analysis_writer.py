...
@tool
def analysis_writer(agent_response: str, agent_name: str, output_directory: str = "analysis_results") -> Dict:
    """
    Analysis Writer Agent - Processes and formats agent responses into structured output files
    
    This function takes the raw response from another agent (like cfn_explorer) and uses an AI agent
    to process, format, and write the results to a JSON file in the specified output directory.
    ...
    try:
        ...
        response = agent(f"Process the response {agent_response} produced by {agent_name} agent and write to the filename {agent_name}_result.json to the directory {output_directory}")
...
```json
{
  ...
  "running_stacks": [
    ...
    },
    {
      ...
      "parameters": [],
      "outputs": {
        "BARSKeyArnExport": "arn:aws:kms:us-east-1:637423355473:key/316e8b3b-ea4a-4be0-87a0-04550570221a"
      },
      "tags": {},
      "resource_details": [
        {
